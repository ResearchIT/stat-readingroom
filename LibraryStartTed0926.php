...
if(isset($opts{'email_overdue_books'})) {
...
function adminScreen($db,$opts,$userName) {
   ...
   print "<P class=\"mypar\"><a href='https://sws.stat.iastate.edu/protected/StaffAccess/ReadingRoom/ReadingRoomBooks.php' target='Books'>Edit Reading Room Books</a><br>";
   print "<a href='https://sws.stat.iastate.edu/protected/StaffAccess/ReadingRoom/ReadingRoomAuthors.php' target='Authors'>Edit Reading Room Authors</a><br>";
   print "<a href='https://sws.stat.iastate.edu/protected/StaffAccess/ReadingRoom/ReadingRoomSearchTerms.php' target='Search'>Edit Reading Room Search Terms</a><br>";
   print "<a href='https://sws.stat.iastate.edu/protected/StaffAccess/ReadingRoom/ReadingRoomHistory.php' target='History'>Edit Reading Room History</a><br></P>";
   booksOut(&$db); 
   booksOverdue(&$db); 
...
function userBooks ($myBooks) {
   ...
   $row = $table->AddRow();
   $table->SetCellContent($row,1,"<strong>Due Date</strong>");
   $table->SetCellContent($row,2,"<strong>Title</strong>");
   $table->SetCellContent($row,3,"<strong>Author</strong>");
...
//      print($date[0]."|".$date[1]."|".$date[2])."<br>";
     ...
     $content = ($returnTime <= $nowTime  ? "<font color=\"#FF0000\">" . $content . "</font" : $content);
     $table->SetCellContent( $row, 1, $content);
     $table->SetCellContent( $row, 2, $book->Title);
     $table->SetCellContent( $row, 3, $book->Author);
   }  
...
// SELECT * FROM `stat`.`ReadingRoomBooks` where DateCheckedOut + CheckOutDays < curdate()
   $query = "SELECT count(*) as count FROM ReadingRoomBooks where DateCheckedOut + CheckOutDays < curdate()"; 
   $count   = $db->get_row($query);
   $query = "SELECT * FROM ReadingRoomBooks where DateCheckedOut + CheckOutDays < curdate()"; ;
   $results   = $db->get_results($query);
   $query = "update ReadingRoomBooks set DateContacted = CURDATE() where DateCheckedOut + CheckOutDays < curdate()"; ;
   $db->query($query);
//   $results   = $db->get_results($query) OR die('Query failed in Library.php. Query = '.$query);
     if($count->count > 0) {
       foreach ($results as $book) {
          ...
          $MailBody = "Please return '".$book->Title."' it is overdue";
...
//   $results   = $db->get_results($query) OR die('Query failed in Library.php. Query = '.$query);
   ...
   $table->SetCellContent( $row, 5, "<strong>Author</strong>");
     ...
     foreach ($results as $book) {
       ...
       $row = $table->AddRow();
       $table->SetCellContent( $row, 1, $book->Borrower );
...
//   $results   = $db->get_results($query) OR die('Query failed in Library.php. Query = '.$query);
   ...
   $table->SetCellContent( $row, 5, "<strong>Author</strong>");
     ...
     foreach ($results as $book) {
       ...
       $table->SetCellContent( $row, 1, $content);
       $table->SetCellContent( $row, 2, $book->Borrower );
...
function checkINBook($db,$user,$book) {
   ...
   $query = "update ReadingRoomBooks set Borrower = NULL,DateCheckedOut = NULL  where BookID = ".$book;
